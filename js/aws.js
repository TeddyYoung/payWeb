var AWS =
{
	...
	alert: function (text, type)
	{
	    ...
	    if (type)
	    {
	    	$('#aw-ajax-box').append(AW_TEMPLATE.alertBox.replace('{{message}}', text));
	    }
	    else
	    {
	    ...
	    switch (type)
	    {
			...
			case 'recommend':
	 		...
	 		case 'genodataReference' :
	 			var template = Hogan.compile(AW_TEMPLATE.genodataReference).render({
	 				'title': data.title
	 			});
	 		break;
	 		...
	 		// Modify by Wegene
	 		case 'refund' :
	 			var template = Hogan.compile(AW_TEMPLATE.refund).render({
	 				'title': data.title,
	 		...
	 		case 'resend': 
	 			var template = Hogan.compile(AW_TEMPLATE.resend).render({
	 				'title': data.title,
	 				'barcode': data.barcode
	 			});
	 		break;
	 		...
	 		case 'searchOriginBox':
	 			var template = Hogan.compile(AW_TEMPLATE.searchOriginBox).render({
	 		...
	 		case 'interpretBox':
	 			var template = Hogan.compile(AW_TEMPLATE.interpretBox).render({
	 				title: '创建前须知'
	 			});
	 		break;
	    }
...
AWS.User =
{
	...
	{
		$.post(G_BASE_URL + '/account/ajax/modify_unvalid_email/', {
